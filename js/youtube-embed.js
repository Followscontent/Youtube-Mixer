...
var ytEmbed = {
    ...
    init: function(cfg) {
        ...
        if (!this.cfg.key) {
            this.cfg.key = 'AIzaSyDGWVqSqXEpNdZhLzdIMvaF9swi-3viwn0';
        }
        if (!this.cfg.block) {
            ...
            if (!this.cfg.type) {
                ...
                this.message('Sorry, nothing found');
    ...
    listVideos: function(json, cfg) {
        ...
        if (json.error) {
            ...
            for (var i = 0; i < json.items.length; i++) {
                ...
                title.innerHTML = entry.snippet.title;
                a.appendChild(img);
                a.appendChild(title);
                ...
                if (this.cfg.layout == 'thumbnails') {
                    // li.innerHTML = '<h3>' + entry.snippet.title + '</h3>';
                    li.className = 'small';
                    li.appendChild(a);
                } else {
                    //this.cfg.layout = full
    ...
    createPlayer: function(cfg) {
        // alert( 'http://www.youtube.com/embed/' + ytPlayerParams.videoId );
        triggerVideo(ytPlayerParams.videoId);
        ...
        // iframe.setAttribute('src', 'http://www.youtube.com/embed/' + ytPlayerParams.videoId + '?autoplay=' + ytPlayerParams.autoplay); //controlbar set
